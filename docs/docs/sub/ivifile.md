...
`ivitools` 是管理 `ivi` 文件的工具，支持：拆开、播放、扫描 `ivi` 文件。`ivitools` 将在 `bilibili-api>=14.0.0.rc` 的版本中自动安装完毕。提供以下命令：

1. `ivitools download [BVID] [OUT]` 下载互动视频至 `ivi` 格式
2. `ivitools extract [IVI] [OUT]` 拆开 `ivi` 文件至目标文件夹
3. `ivitools help` 帮助
4. `ivitools play [IVI]` 播放 `ivi` 文件 (`linux` 用户需将 `ffmpeg` 加入环境变量) (需要 `PyQt6` 作为 GUI 库)
5. `ivitools scan [IVI]` 扫描 `ivi` 文件
6. `ivitools touch [IVI]` 获取 `ivi` 文件简介 (`JSON` 格式 )

## `ivi` 文件详解

解析 `ivi` 文件首先需要拆开它，可以用 `ivitools extract` 命令，当然，`ivi` 文件的打包格式就是 `zip`，可以将后缀改成 `zip` 后直接解压。
...
根据目前已知的互动视频机制，及部分互动视频的测试，可以确认目前 `ivi` 文件可以还原以下的功能：
